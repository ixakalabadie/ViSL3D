...
class write_html:
    ...
    def func_colormaps(self, l_isolevels):
        ...
        for nc in range(numcubes):
            ...
            self.file_html.write(tabs(4)+"var cmap = cc0.value;\n")
            self.file_html.write(tabs(4)+"var min = cmapmin.value;\n")
            self.file_html.write(tabs(4)+"var max = cmapmax.value;\n")
            self.file_html.write(tabs(4)+"const scale = cmapscale.value;\n")
            self.file_html.write(tabs(4)+"var reverse = false;\n")
            self.file_html.write(tabs(4)+"var collevs = [];\n")

            self.file_html.write(tabs(4)+"if (scale === 'linear') {\n")
            self.file_html.write(tabs(5)+"for (const level of isolevels) {\n")
            self.file_html.write(tabs(6)+"collevs.push(level);\n")
            self.file_html.write(tabs(5)+"}\n")
            self.file_html.write(tabs(4)+"}\n")
            self.file_html.write(tabs(4)+"else if (scale === 'log') {\n")
            self.file_html.write(tabs(5)+"min = Math.log(min);\n")
            self.file_html.write(tabs(5)+"max = Math.log(max);\n")
            self.file_html.write(tabs(5)+"for (const level of isolevels) {\n")
            self.file_html.write(tabs(6)+"collevs.push(Math.log(level));\n")
            self.file_html.write(tabs(5)+"}\n")
            self.file_html.write(tabs(4)+"}\n")
            self.file_html.write(tabs(4)+"else if (scale === 'sqrt') {\n")
            self.file_html.write(tabs(5)+"min = Math.sqrt(min);\n")
            self.file_html.write(tabs(5)+"max = Math.sqrt(max);\n")
            self.file_html.write(tabs(5)+"for (const level of isolevels) {\n")
            self.file_html.write(tabs(6)+"collevs.push(Math.sqrt(level));\n")
            self.file_html.write(tabs(5)+"}\n")
            self.file_html.write(tabs(4)+"}\n")
            self.file_html.write(tabs(4)+"else if (scale === 'power') {\n")
            self.file_html.write(tabs(5)+"min = Math.pow(min, 2);\n")
            self.file_html.write(tabs(5)+"max = Math.pow(max, 2);\n")
            self.file_html.write(tabs(5)+"for (const level of isolevels) {\n")
            self.file_html.write(tabs(6)+"collevs.push(Math.pow(level, 2));\n")
            self.file_html.write(tabs(5)+"}\n")
            self.file_html.write(tabs(4)+"}\n")
            self.file_html.write(tabs(4)+"else if (scale === 'asinh') {\n")
            self.file_html.write(tabs(5)+"min = Math.asinh(min);\n")
            self.file_html.write(tabs(5)+"max = Math.asinh(max);\n")
            self.file_html.write(tabs(5)+"for (const level of isolevels) {\n")
            self.file_html.write(tabs(6)+"collevs.push(Math.asinh(level));\n")
            self.file_html.write(tabs(5)+"}\n")
            self.file_html.write(tabs(4)+"}\n")
            self.file_html.write(tabs(4)+"for (let cub = 0; cub < numcubes; cub++) {\n")
            self.file_html.write(tabs(5)+"for (let lev = 0; lev < isolevels.length; lev++) {\n")
            self.file_html.write(tabs(6)+"if (min >= collevs[lev]) {\n")
            self.file_html.write(tabs(7)+"collevs[lev] = 0;\n")
            self.file_html.write(tabs(6)+"} else if (max <= collevs[lev]) {\n")
            self.file_html.write(tabs(7)+"collevs[lev] = 1;\n")
            self.file_html.write(tabs(6)+"} else {\n")
            self.file_html.write(tabs(7)+"collevs[lev] = (collevs[lev] - min) / (max - min);\n")
            self.file_html.write(tabs(6)+"}\n")
            self.file_html.write(tabs(6)+"if (cmap.endsWith('_r')) {\n")
            self.file_html.write(tabs(7)+"reverse = true\n")
            self.file_html.write(tabs(6)+"}\n")
            self.file_html.write(tabs(6)+"const color = evaluate_cmap(collevs[lev], cmap.replace('_r', ''), reverse);\n")
            self.file_html.write(tabs(6)+"document.getElementById('cube__'+cub+'layer'+lev).setAttribute('diffuseColor', color[0]/255+' '+color[1]/255+' '+color[2]/255);\n")
            self.file_html.write(tabs(6)+"document.getElementById(cub+'but'+lev).style.background = 'rgb('+color[0]+' '+color[1]+' '+color[2]+')';\n")
            self.file_html.write(tabs(6)+"if ((color[0]*0.299 + color[1]*0.587 + color[2]*0.114) > 130) {\n")
            self.file_html.write(tabs(7)+"document.getElementById(cub+'but'+lev).style.color = 'black';\n")
            self.file_html.write(tabs(6)+"} else {\n")
            self.file_html.write(tabs(7)+"document.getElementById(cub+'but'+lev).style.color = 'white';\n")
            self.file_html.write(tabs(6)+"}\n")
            self.file_html.write(tabs(5)+"}\n")
            self.file_html.write(tabs(4)+"}\n")
            self.file_html.write(tabs(3)+"}\n")
