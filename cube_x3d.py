...
class write_html:
    ...
    def __init__(self, filename, units, tabtitle='new_html_x3d', pagetitle=None, description=None, ntubes=0):
    ...
    def func_tubes(self):
        ...
        Run after buttons()
        """
        self.write_html(tabs(2) + '<div id="divmaster" style="margin-left: 2%">\n')
        self.write_html(tabs(3) + '<!-- BUTTON TO CHANGE LAYOUT -->\n')
        self.write_html(tabs(3) + '<label for="markers-choice"><b>Markers:</b> </label>\n')
        self.write_html(tabs(3) + '<select id="markers-choice">\n')
        self.write_html(tabs(4) + '<option value="none">None</option>\n')
        self.write_html(tabs(4) + '<option value="sphere">Sphere</option>\n')
        self.write_html(tabs(4) + '<option value="box">Box</option>\n')
        self.write_html(tabs(4) + '<option value="tube">Tube</option>\n')
        self.write_html(tabs(4) + '<option value="cone">Cone</option>\n')
        self.write_html(tabs(3) + '</select>\n')
        self.write_html(tabs(3) + '<input type="color" id="butcol" value="#ff0000">\n')
        self.write_html(tabs(3) + '<button id="butcreate" onclick="createmarker()">Create</button>\n')
        self.write_html(tabs(3) + '<button id="butremove" onclick="removemarker()">Remove</button> <br><br>\n')
        self.write_html(tabs(2) + '</div>\n')
        self.write_html(tabs(2) + '<!-- create various layouts for different objects -->\n')
        self.write_html(tabs(2) + '<div id="spherediv" style="display:none ; margin-left: 2%">\n')
        self.write_html(tabs(3) + '<input type="number" id="sphX0" min="-284" max="284" step="1" placeholder="RA">\n')
        self.write_html(tabs(3) + '<input type="number" id="sphY0" min="-232" max="232" step="1" placeholder="Dec">\n')
        self.write_html(tabs(3) + '<input type="number" id="sphZ0" min="-259" max="259" step="1" placeholder="V">\n')
        self.write_html(tabs(3) + '<input type="number" id="sphrad0" min="0" max="50" step="0.5" placeholder="Radius">\n')
        self.write_html(tabs(2) + '</div>\n')
        self.write_html(tabs(2) + '<div id="boxdiv" style="display:none ; margin-left: 2%">\n')
        self.write_html(tabs(3) + '<input type="number" id="boxX0" min="-284" max="284" step="1" placeholder="RA">\n')
        self.write_html(tabs(3) + '<input type="number" id="boxY0" min="-232" max="232" step="1" placeholder="Dec">\n')
        self.write_html(tabs(3) + '<input type="number" id="boxZ0" min="-259" max="259" step="1" placeholder="V">\n')
        self.write_html(tabs(3) + '<input type="text" id="boxrad0" placeholder="shape, e.g. \'20 20 20\'">\n')
        self.write_html(tabs(2) + '</div>\n')
        self.write_html(tabs(2) + '<div id="tubediv" style="display:none ; margin-left: 2% ; width: 80%">\n')
        self.write_html(tabs(3) + '<br>\n')
        self.write_html(tabs(3) + '<button id="newtube" onclick="newtube()">New</button>\n')
        self.write_html(tabs(3) + '<button id="addpoint" onclick="addpoint()">Add point</button>\n')
        self.write_html(tabs(3) + '<select id="new-tubes">\n')
        self.write_html(tabs(4) + '<option value="none">None</option>\n')
        self.write_html(tabs(3) + '</select>\n')
        self.write_html(tabs(3) + '<br>\n')
        self.write_html(tabs(2) + '</div>\n')

        self.write_html(tabs(2)+'<script>')
        self.write_html(tabs(3) + 'var ngeo = 0; //number of new geometrical shapes\n')
        self.write_html(tabs(3) + 'var tubelen = []; //number of cylinders in each tube\n')
        self.write_html(tabs(3) + 'var npoints = 0; // number of points of a tube (number of cylinders -1)(is changed for each tube)\n')
        self.write_html(tabs(3) + 'const marktype = document.querySelector(\'#markers-choice\');\n')
        self.write_html(tabs(3) + 'marktype.addEventListener(\'change\', newlayout);\n')
        self.write_html(tabs(3) + 'const selTube = document.querySelector(\'#new-tubes\');\n')
        self.write_html(tabs(3) + 'selTube.addEventListener(\'change\', changeTube)\n')
        self.write_html(tabs(3) + 'function addpoint() {\n')
        self.write_html(tabs(4) + 'const tubeIndex = selTube.value.split(\'\');\n')
        self.write_html(tabs(4) + 'const tubeInd = tubeIndex[tubeIndex.length-1];\n')
        self.write_html(tabs(4) + 'npoints = tubelen[tubeInd] + 2; // add 2 to add one point more. tubelen gives the number of cylinders.\n')
        self.write_html(tabs(4) + 'const newra = document.createElement(\'input\');\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'type\', \'number\');\n')
        ...
        self.write_html(tabs(4) + 'const newra = document.createElement(\'input\');\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'id\', \'tub\'+tubeInd+\'X\'+(npoints-1));\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'placeholder\', \'RA\');\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'min\', \'-284\');\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'max\', \'284\');\n')
        self.write_html(tabs(4) + 'newra.setAttribute(\'step\', \'1\');\n')
        self.write_html(tabs(4) + '\n')
        self.write_html(tabs(4) + 'const newdec = document.createElement(\'input\');\n')
        self.write_html(tabs(4) + 'newdec.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(4) + 'newdec.setAttribute(\'id\', \'tub\'+tubeInd+\'Y\'+' '(npoints-1));\n')
        self.write_html(tabs(4) + 'newdec.setAttribute(\'placeholder\', \'Dec\');\n')
        self.write_html(tabs(4) + 'newdec.setAttribute(\'min\', \'-232\');\n')
        self.write_html(tabs(4) + 'newdec.setAttribute(\'max\', \'232\');\n')
        self.write_html(tabs(4) + 'newdec.setAttribute(\'step\', \'1\');\n')
        self.write_html(tabs(4) + '\n')
        self.write_html(tabs(4) + 'const newv = document.createElement(\'input\');\n')
        self.write_html(tabs(4) + 'newv.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(4) + 'newv.setAttribute(\'id\', \'tub\'+tubeInd+\'Z\'+' '(npoints-1));\n')
        self.write_html(tabs(4) + 'newv.setAttribute(\'placeholder\', \'V\');\n')
        self.write_html(tabs(4) + 'newv.setAttribute(\'min\', \'-259\');\n')
        self.write_html(tabs(4) + 'newv.setAttribute(\'max\', \'259\');\n')
        self.write_html(tabs(4) + 'newv.setAttribute(\'step\', \'1\');\n')
        self.write_html(tabs(4) + 'const newline = document.createElement("br");\n')
        self.write_html(tabs(4) + 'document.getElementById(\'div\'+selTube.value).appendChild(newline);\n')
        self.write_html(tabs(4) + 'document.getElementById(\'div\'+selTube.value).appendChild(newra);\n')
        self.write_html(tabs(5) + '//document.getElementById(\'tubediv\').insertBefore(newra, null); //document.getElementById(\'addp\')\n')
        self.write_html(tabs(4) + 'document.getElementById(\'div\'+selTube.value).appendChild(newdec);\n')
        self.write_html(tabs(5) + '//document.getElementById(\'tubediv\').insertBefore(newdec, null);\n')
        self.write_html(tabs(4) + 'document.getElementById(\'div\'+selTube.value).appendChild(newv);\n')
        self.write_html(tabs(5) + '//document.getElementById(\'tubediv\').insertBefore(newv, null);\n')
        self.write_html(tabs(4) + '\n')
        self.write_html(tabs(4) + 'tubelen[tubeInd] = npoints - 1;\n')
        self.write_html(tabs(3) + '}\n')
        self.write_html(tabs(4) + '\n')

        self.write_html(tabs(3) + 'function newtube() {\n')
        self.write_html(tabs(4) + 'tubelen.push(1);\n')
        self.write_html(tabs(4) + 'const l = tubelen.length-1;\n')
        self.write_html(tabs(4) + '\n')
        self.write_html(tabs(4) + 'if (selTube.value != \'none\') {\n')
        self.write_html(tabs(5) + 'document.getElementById(\'div\'+selTube.value).style.display = \'none\'\n')
        self.write_html(tabs(5) + 'const len = document.getElementById("new-tubes").length\n')
        self.write_html(tabs(5) + 'document.getElementById("new-tubes")[len] = new Option(\'tube\'+l, \'tube\'+l, true, true);\n')
        self.write_html(tabs(4) + '} else {\n')
        self.write_html(tabs(5) + 'document.getElementById("new-tubes")[0] = new Option(\'tube\'+l, \'tube\'+l, true, true); //this value is selected in the input button\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(4) + '// move tubediv0 here. use new button to create new div, no coordinate inputs from start\n')
        self.write_html(tabs(4) + 'const newdiv = document.createElement(\'div\');\n')
        self.write_html(tabs(4) + 'document.getElementById(\'tubediv\').appendChild(newdiv);\n')
        self.write_html(tabs(4) + 'newdiv.setAttribute(\'id\', \'divtube\'+l);\n')
        self.write_html(tabs(4) + 'newdiv.setAttribute(\'style\', \'margin-left: 2%\');\n')
        self.write_html(tabs(4) + 'newdiv.appendChild(document.createElement("br"));\n')
        self.write_html(tabs(4) + '\n')
        self.write_html(tabs(4) + '// create different radius for each tube\n')
        self.write_html(tabs(4) + '//<input type="number" id="tubrad" min="0" max="50" step="0.5" placeholder="Radius">\n')
        self.write_html(tabs(4) + 'const newrad = document.createElement(\'input\');\n')
        self.write_html(tabs(4) + 'newrad.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(4) + 'newrad.setAttribute(\'id\', \'tubrad\'+l);\n')
        self.write_html(tabs(4) + 'newrad.setAttribute(\'min\', \'0\');\n')
        self.write_html(tabs(4) + 'newrad.setAttribute(\'max\', \'50\');\n')
        self.write_html(tabs(4) + 'newrad.setAttribute(\'step\', \'0.5\');\n')
        self.write_html(tabs(4) + 'newrad.setAttribute(\'placeholder\', \'Radius\');\n')
        self.write_html(tabs(4) + 'newdiv.appendChild(newrad);\n')
        self.write_html(tabs(4) + '\n')
        self.write_html(tabs(4) + 'for (i=0 ; i<2 ; i++) {\n')
        self.write_html(tabs(5) + 'const newra = document.createElement(\'input\');\n')
        self.write_html(tabs(5) + 'newra.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(5) + 'newra.setAttribute(\'id\', \'tub\'+l+\'X\'+i);\n')
        self.write_html(tabs(5) + 'newra.setAttribute(\'placeholder\', \'RA\');\n')
        self.write_html(tabs(5) + 'newra.setAttribute(\'step\', \'1\');\n')
        self.write_html(tabs(5) + '\n')
        self.write_html(tabs(5) + 'const newdec = document.createElement(\'input\');\n')
        self.write_html(tabs(5) + 'newdec.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(5) + 'newdec.setAttribute(\'id\', \'tub\'+l+\'Y\'+i);\n')
        self.write_html(tabs(5) + 'newdec.setAttribute(\'placeholder\', \'Dec\');\n')
        self.write_html(tabs(5) + 'newdec.setAttribute(\'step\', \'1\');\n')
        self.write_html(tabs(5) + '\n')
        self.write_html(tabs(5) + 'const newv = document.createElement(\'input\');\n')
        self.write_html(tabs(5) + 'newv.setAttribute(\'type\', \'number\');\n')
        self.write_html(tabs(5) + 'newv.setAttribute(\'id\', \'tub\'+l+\'Z\'+i);\n')
        self.write_html(tabs(5) + 'newv.setAttribute(\'placeholder\', \'V\');\n')
        self.write_html(tabs(5) + 'newv.setAttribute(\'step\', \'1\');\n')
        self.write_html(tabs(5) + '\n')
        self.write_html(tabs(5) + 'const newline = document.createElement("br");\n')
        self.write_html(tabs(5) + 'newdiv.appendChild(newline);\n')
        self.write_html(tabs(5) + 'newdiv.appendChild(newra);\n')
        self.write_html(tabs(5) + 'newdiv.appendChild(newdec);\n')
        self.write_html(tabs(5) + 'newdiv.appendChild(newv);\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(3) + '}\n')

        self.write_html(tabs(3) + 'function newlayout() {\n')
        self.write_html(tabs(4) + 'document.getElementById(\'spherediv\').style.display = \'none\'\n')
        self.write_html(tabs(4) + 'document.getElementById(\'boxdiv\').style.display = \'none\'\n')
        self.write_html(tabs(4) + 'document.getElementById(\'tubediv\').style.display = \'none\'\n')
        self.write_html(tabs(4) + 'name = marktype.value+\'div\'\n')
        self.write_html(tabs(4) + 'document.getElementById(name).style.display = \'inline-block\'\n')
        self.write_html(tabs(3) + '}\n')

        self.write_html(tabs(3) + 'function createmarker() {\n')
        self.write_html(tabs(4) + 'const tubeIndex = selTube.value.split(\'\');\n')
        self.write_html(tabs(4) + 'const tubeInd = tubeIndex[tubeIndex.length-1];\n')
        self.write_html(tabs(4) + 'npoints = tubelen[tubeInd] + 1\n')
        self.write_html(tabs(4) + 'if (document.getElementById(tubeInd+\'newtra0\') == null) {\n')
        self.write_html(tabs(5) + 'for (i=0; i<npoints-1; i++) {\n')
        self.write_html(tabs(6) + 'const x0 = Number(document.querySelector(\'#tub\'+tubeInd+\'X\'+i).value);\n')
        self.write_html(tabs(6) + 'const y0 = Number(document.querySelector(\'#tub\'+tubeInd+\'Y\'+i).value);\n')
        self.write_html(tabs(6) + 'const z0 = Number(document.querySelector(\'#tub\'+tubeInd+\'Z\'+i).value);\n')
        self.write_html(tabs(6) + 'const x1 = Number(document.querySelector(\'#tub\'+tubeInd+\'X\'+(i+1)).value);\n')
        self.write_html(tabs(6) + 'const y1 = Number(document.querySelector(\'#tub\'+tubeInd+\'Y\'+(i+1)).value);\n')
        self.write_html(tabs(6) + 'const z1 = Number(document.querySelector(\'#tub\'+tubeInd+\'Z\'+(i+1)).value);\n')
        self.write_html(tabs(6) + 'const rad = document.querySelector(\'#tubrad\'+tubeInd);\n')
        self.write_html(tabs(6) + 'const trans = [(x0+x1)/2, (y0+y1)/2, (z0+z1)/2]\n')
        self.write_html(tabs(6) + 'const diff = [x1-x0, y1-y0, z1-z0]\n')
        self.write_html(tabs(6) + 'const height = Math.sqrt(diff[0]**2+diff[1]**2+diff[2]**2);\n')
        self.write_html(tabs(6) + 'const angle = Math.acos(diff[1]/height);\n')
        self.write_html(tabs(6) + 'const newtra = document.createElement(\'transform\');\n')
        self.write_html(tabs(6) + 'newtra.setAttribute(\'id\', tubeInd+\'newtra\'+i);\n')
        self.write_html(tabs(6) + 'const newshape = document.createElement(\'shape\');\n')
        self.write_html(tabs(6) + 'newshape.setAttribute(\'id\', tubeInd+\'newsha\'+i);\n')
        self.write_html(tabs(6) + 'const newape = document.createElement(\'appearance\');\n')
        self.write_html(tabs(6) + 'const newmat = document.createElement(\'material\');\n')
        self.write_html(tabs(6) + 'newmat.setAttribute(\'diffuseColor\', col.value);\n')
        self.write_html(tabs(6) + 'newmat.setAttribute(\'id\', tubeInd+\'mat\'+i);\n')
        self.write_html(tabs(6) + 'const newgeo = document.createElement(\'cylinder\');\n')
        self.write_html(tabs(6) + 'newgeo.setAttribute(\'id\', tubeInd+\'newcyl\'+i);\n')
        self.write_html(tabs(6) + 'newgeo.setAttribute(\'radius\', rad.value);\n')
        self.write_html(tabs(6) + 'newgeo.setAttribute(\'solid\', \'false\');\n')
        self.write_html(tabs(6) + 'newgeo.setAttribute(\'height\', height.toString());\n')
        self.write_html(tabs(6) + 'newtra.setAttribute(\'translation\', trans.toString());\n')
        self.write_html(tabs(6) + 'newtra.setAttribute(\'rotation\', diff[2]+\' 0 \'+(-diff[0])+\' \'+angle);\n')
        self.write_html(tabs(6) + 'newshape.appendChild(newape).appendChild(newmat);\n')
        self.write_html(tabs(6) + 'newtra.appendChild(newshape).appendChild(newgeo);\n')
        self.write_html(tabs(6) + 'document.getElementById(\'cube__ROOT\').appendChild(newtra);\n')
        self.write_html(tabs(5) + '}\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(4) + 'else {\n')
        self.write_html(tabs(5) + 'for (i=0; i<npoints-1; i++) {\n')
        ...
        self.write_html(tabs(6) + 'const height = Math.sqrt(diff[0]**2+diff[1]**2+diff[2]**2);\n')
        self.write_html(tabs(6) + 'const angle = Math.acos(diff[1]/height);\n')
        self.write_html(tabs(6) + 'document.getElementById(tubeInd+\'newtra\'+i).setAttribute(\'rotation\', diff[2]+\' 0 \'+(-diff[0])+\' \'+angle);\n')
        self.write_html(tabs(6) + 'document.getElementById(tubeInd+\'newtra\'+i).setAttribute(\'translation\', trans.toString());\n')
        self.write_html(tabs(6) + 'document.getElementById(tubeInd+\'newcyl\'+i).setAttribute(\'height\', height.toString());\n')
        self.write_html(tabs(6) + 'document.getElementById(tubeInd+\'newcyl\'+i).setAttribute(\'radius\', rad.value);\n')
        self.write_html(tabs(5) + '}\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(3) + '}\n')

        self.write_html(tabs(3) + 'function removemarker() {\n')
        self.write_html(tabs(4) + 'const tubeIndex = selTube.value.split(\'\');\n')
        self.write_html(tabs(4) + 'const tubeInd = tubeIndex[tubeIndex.length-1];\n')
        self.write_html(tabs(4) + 'npoints = tubelen[tubeInd] + 1\n')
        self.write_html(tabs(4) + 'if (tubelen.length === 1) {\n')
        self.write_html(tabs(5) + 'document.getElementById("new-tubes")[tubeInd] = new Option(\'None\', \'none\', false, false);\n')
        self.write_html(tabs(4) + '} else {\n')
        self.write_html(tabs(5) + 'document.getElementById("new-tubes")[tubeInd].remove();\n')
        self.write_html(tabs(5) + 'document.getElementById("new-tubes")[0].setAttribute(\'selected\', \'selected\');\n')
        self.write_html(tabs(5) + 'const nexttube = document.getElementById("new-tubes")[0].value;\n')
        self.write_html(tabs(5) + 'document.getElementById(\'div\'+nexttube).style.display = \'inline-block\';\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(4) + 'for (i=0; i<npoints-1; i++){\n')
        self.write_html(tabs(5) + 'document.getElementById(tubeInd+\'newtra\'+i).remove();\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(4) + 'document.getElementById(\'divtube\'+tubeInd).remove();\n')
        self.write_html(tabs(3) + '}\n')

        self.write_html(tabs(3) + 'function changeTube() {\n')
        self.write_html(tabs(4) + 'for (i=0; i<tubelen.length; i++) {\n')
        self.write_html(tabs(5) + 'alert(selTube.value)\n')
        self.write_html(tabs(5) + 'if (\'tube\'+i != selTube.value) {\n')
        self.write_html(tabs(6) + 'if (document.getElementById(\'divtube\'+i) != null) {\n')
        self.write_html(tabs(7) + 'document.getElementById(\'divtube\'+i).style.display = \'none\';\n')
        self.write_html(tabs(6) + '}\n')
        self.write_html(tabs(5) + '}\n')
        self.write_html(tabs(5) + 'document.getElementById(\'div\'+selTube.value).style.display = \'inline-block\';\n')
        self.write_html(tabs(4) + '}\n')
        self.write_html(tabs(3) + '}\n')
    ...
    def buttons(self, l_isolevels=None, l_colors=None, colormaps=None, hide2d=False, scalev=False, move2d=False, lineLabs=False, centRot=False, background=False):
        ...
        if self.grids or self.gals or self.gallabs or self.axes or hide2d or self.tubes or self.boxbut or self.sphbut:
        ...
        if self.tubes:
            self.file_html.write(tabs(3)+'<button onclick="settubes();" >Tube</button>\n')
        if self.boxbut:
            self.file_html.write(tabs(3)+'<button onclick="setbox();" >Boxes</button>\n')
        if self.sphbut:
            self.file_html.write(tabs(3)+'<button onclick="setsph();" >Spheres</button>\n')
    ...
    def func_scalev(self, coords, gals=None, axes='both', move2d=True, points=None, box=None, sph=None):
        """
        ...
        if points is not None:
            trans = np.array([np.mean((points[i],points[i+1]), axis=0) for i in range(len(points)-1)])
            diff = np.diff(points, axis=0)
            for i in range(len(points)-1):
                ...
                if diff[i,2] != 0:
                    if diff[i,1] == 0:
                        self.file_html.write(tabs(3)+"const norm%s = Math.sqrt(%s+(sca*%s)**2);\n"%(i,diff[i,0]**2,diff[i,2]))
                        self.file_html.write(tabs(3)+"document.getElementById('cube__tubtra%s').setAttribute('rotation', sca*%s+' 0 %s 1.57079633');\n"%(i,diff[i,2],-diff[i,0]))
                    else:
                        self.file_html.write(tabs(3)+"const norm%s = Math.sqrt(%s+(sca*%s)**2);\n"%(i,diff[i,0]**2+diff[i,1]**2,diff[i,2]))
                        self.file_html.write(tabs(3)+"const angley%s = Math.acos(%s/norm%s);\n"%(i,diff[i,1],i))
                        self.file_html.write(tabs(3)+"document.getElementById('cube__tubtra%s').setAttribute('rotation', sca*%s+' 0 %s '+angley%s);\n"%(i,diff[i,2],-diff[i,0],i))
                    self.file_html.write(tabs(3)+"document.getElementById('cube__tube%s').setAttribute('height', norm%s*1.05);\n"%(i,i))
                    
        #box always changes scale and translation, sphere only translation
        if box is not None:
            for i in range(len(box)):
                self.file_html.write(tabs(3)+"document.getElementById('cube__boxtra%s').setAttribute('translation', '%s %s '+sca*%s);\n"%(i,box[i,0],box[i,1],box[i,2]))
                self.file_html.write(tabs(3)+"document.getElementById('cube__boxtra%s').setAttribute('scale','1 1 '+sca);\n"%i)
        
        if sph is not None:
            for i in range(len(sph)):
                self.file_html.write(tabs(3)+"document.getElementById('cube__sphtra%s').setAttribute('translation', '%s %s '+sca*%s);\n"%(i,sph[i,0],sph[i,1],sph[i,2]))
