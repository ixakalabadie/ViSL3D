...
class write_x3d:
    ...
    def make_ticklines(self):
        ...
        self.file_x3d.write('\n\t\t\t\t\t<Appearance>')
        #set color
        self.file_x3d.write('\n\t\t\t\t\t\t<Material DEF="ticklines"  emissiveColor="%s" diffuseColor="0 0 0"/>'%self.col)
        self.file_x3d.write('\n\t\t\t\t\t</Appearance>')
        self.file_x3d.write('\n\t\t\t\t\t<IndexedLineSet colorPerVertex="false" coordIndex="\n\t\t\t\t\t\t')
        #write indices
...
class write_html:
    ...
    def func_tubes(self):
        """
        Run after buttons(). It is assumed that scalev is used. It will fail otherwise. To use without scalev, remove(modify parts with sca).
        """
        self.intmarks = True
        self.file_html.write(tabs(2) + '\n')
    ...
    def buttons(self, l_isolevels=None, l_colors=None, colormaps=None, hide2d=False, scalev=False, move2d=False, lineLabs=False, centRot=False, background=False):
        ...
        if colormaps is not None:
            ...
            for nc in range(numcubes):
                ...
                self.write_html(tabs(2) + '<label for="cmaps-min%s"><b>Min: %s</b> </label>\n'%(nc,nc))
                self.write_html(tabs(2) + '<input id="cmaps-min" type="number" value="3">\n')
                self.write_html(tabs(2) + '<label for="cmaps-max%s"><b>Max: %s</b> </label>\n'%(nc,nc))
                self.write_html(tabs(2) + '<input id="cmaps-max" type="number" value="15">\n')
                self.write_html(tabs(2) + '<label for="cmaps-min%s"><b>Scale %s:</b> </label>\n'%(nc,nc))
                self.write_html(tabs(2) + '<select id="cmaps-scale%s">\n'%nc)
                self.write_html(tabs(3) + '<option value="linear" selected="selected">linear</option>\n')
                self.write_html(tabs(3) + '<option value="log">log</option>\n')
                self.write_html(tabs(3) + '<option value="sqrt">sqrt</option>\n')
                self.write_html(tabs(3) + '<option value="power">power</option>\n')
                self.write_html(tabs(3) + '<option value="asinh">asinh</option>\n')
                self.write_html(tabs(2) + '</select>\n')
    ...
    def func_colormaps(self, l_isolevels):
        ...
        for nc in range(numcubes):
            ...
            self.file_html.write(tabs(3)+"const cc0 = document.querySelector('#cmaps-choice0');\n")
			self.file_html.write(tabs(3)+"cc0.addEventListener('change', change_colormap);\n")
			self.file_html.write(tabs(3)+"const cmapmin = document.querySelector('#cmaps-min');\n")
			self.file_html.write(tabs(3)+"cmapmin.addEventListener('change', change_colormap);\n")
			self.file_html.write(tabs(3)+"const cmapmax = document.querySelector('#cmaps-max');\n")
			self.file_html.write(tabs(3)+"cmapmax.addEventListener('change', change_colormap);\n")
			self.file_html.write(tabs(3)+"const cmapscale = document.querySelector('#cmaps-scale');\n")
			self.file_html.write(tabs(3)+"cmapscale.addEventListener('change', change_colormap);\n")
			self.file_html.write(tabs(3)+"const isolevels = [3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0, 8.0, 9.0, 10., 12.5, 15.0];\n")
			self.file_html.write(tabs(3)+"const numcubes = 1;\n")

			self.file_html.write(tabs(3)+"function change_colormap() {\n")
				self.file_html.write(tabs(3)+"var cmap = cc0.value;\n")
				self.file_html.write(tabs(3)+"var min = cmapmin.value;\n")
				self.file_html.write(tabs(3)+"var max = cmapmax.value;\n")
				self.file_html.write(tabs(3)+"const scale = cmapscale.value;\n")
				self.file_html.write(tabs(3)+"var reverse = false;\n")
				self.file_html.write(tabs(3)+"var collevs = [];\n")

				self.file_html.write(tabs(3)+"if (scale === 'linear') {\n")
					self.file_html.write(tabs(3)+"for (const level of isolevels) {\n")
						self.file_html.write(tabs(3)+"collevs.push(level);\n")
					self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"else if (scale === 'log') {\n")
					self.file_html.write(tabs(3)+"min = Math.log(min);\n")
					self.file_html.write(tabs(3)+"max = Math.log(max);\n")
					self.file_html.write(tabs(3)+"for (const level of isolevels) {\n")
						self.file_html.write(tabs(3)+"collevs.push(Math.log(level));\n")
					self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"else if (scale === 'sqrt') {\n")
					self.file_html.write(tabs(3)+"min = Math.sqrt(min);\n")
					self.file_html.write(tabs(3)+"max = Math.sqrt(max);\n")
					self.file_html.write(tabs(3)+"for (const level of isolevels) {\n")
						self.file_html.write(tabs(3)+"collevs.push(Math.sqrt(level));\n")
					self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"else if (scale === 'power') {\n")
					self.file_html.write(tabs(3)+"min = Math.pow(min, 2);\n")
					self.file_html.write(tabs(3)+"max = Math.pow(max, 2);\n")
					self.file_html.write(tabs(3)+"for (const level of isolevels) {\n")
						self.file_html.write(tabs(3)+"collevs.push(Math.pow(level, 2));\n")
					self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"else if (scale === 'asinh') {\n")
					self.file_html.write(tabs(3)+"min = Math.asinh(min);\n")
					self.file_html.write(tabs(3)+"max = Math.asinh(max);\n")
					self.file_html.write(tabs(3)+"for (const level of isolevels) {\n")
						self.file_html.write(tabs(3)+"collevs.push(Math.asinh(level));\n")
					self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"for (let cub = 0; cub < numcubes; cub++) {\n")
					self.file_html.write(tabs(3)+"for (let lev = 0; lev < isolevels.length; lev++) {\n")
						self.file_html.write(tabs(3)+"if (min >= collevs[lev]) {\n")
							self.file_html.write(tabs(3)+"collevs[lev] = 0;\n")
						self.file_html.write(tabs(3)+"} else if (max <= collevs[lev]) {\n")
							self.file_html.write(tabs(3)+"collevs[lev] = 1;\n")
						self.file_html.write(tabs(3)+"} else {\n")
							self.file_html.write(tabs(3)+"collevs[lev] = (collevs[lev] - min) / (max - min);\n")
						self.file_html.write(tabs(3)+"}\n")
						self.file_html.write(tabs(3)+"if (cmap.endsWith('_r')) {\n")
							self.file_html.write(tabs(3)+"reverse = true\n")
						self.file_html.write(tabs(3)+"}\n")
						self.file_html.write(tabs(3)+"const color = evaluate_cmap(collevs[lev], cmap.replace('_r', ''), reverse);\n")
						self.file_html.write(tabs(3)+"document.getElementById('cube__'+cub+'layer'+lev).setAttribute('diffuseColor', color[0]/255+' '+color[1]/255+' '+color[2]/255);\n")
						self.file_html.write(tabs(3)+"document.getElementById(cub+'but'+lev).style.background = 'rgb('+color[0]+' '+color[1]+' '+color[2]+')';\n")
						self.file_html.write(tabs(3)+"if ((color[0]*0.299 + color[1]*0.587 + color[2]*0.114) > 130) {\n")
							self.file_html.write(tabs(3)+"document.getElementById(cub+'but'+lev).style.color = 'black';\n")
						self.file_html.write(tabs(3)+"} else {\n")
							self.file_html.write(tabs(3)+"document.getElementById(cub+'but'+lev).style.color = 'white';\n")
						self.file_html.write(tabs(3)+"}\n")
					self.file_html.write(tabs(3)+"}\n")
				self.file_html.write(tabs(3)+"}\n")
			self.file_html.write(tabs(3)+"}\n")
            ...
            for (i,cmap) in enumerate(self.colormaps):
                ...
                for lev in range(len(l_isolevels[nc])):
                    ...
                    #might be only useful for opaque mode
